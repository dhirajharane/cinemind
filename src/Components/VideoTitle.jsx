...
const VideoTitle = ({ title, overview, movieId }) => {
  ...
  return (
    <div className="absolute top-0 left-0 w-full sm:h-full px-2 sm:px-16 pt-[34%] sm:pt-[28%] text-white bg-gradient-to-r from-black via-black/30 to-transparent h-[256px]">
      <h1 className="text-sm sm:text-6xl font-bold drop-shadow-xl mb-2 sm:mb-6">{title}</h1>
      <p className="text-xs sm:text-xl font-normal sm:font-medium max-w-xs sm:max-w-xl drop-shadow-md leading-relaxed mb-2 sm:mb-8">
        {overview}
      </p>
      <div className="flex flex-row sm:flex-row items-start sm:items-center gap-2 sm:gap-4">
        ...
        <button
          className="bg-gray-600/80 text-white text-xs sm:text-lg font-medium sm:font-semibold px-1 sm:px-8 py-0.5 sm:py-3 rounded-md hover:bg-gray-500/80 transition-all flex items-center gap-1 sm:gap-3 sm:mt-0"
          onClick={() => navigate(`/moviedetails/${movieId}`)}
        >
